...
```{r read-updated-data, eval=F}
#udpated data
summary_report <- read_csv(here("data/annual_summary_report.csv"))
#names(summary_report) <- tolower(names(summary_report))
```
...
plot_data <- summary_report %>% 
  filter(Year == 2018) %>% 
  mutate(Visitors = RecreationVisitors + NonRecreationVisitors) %>% 
  select(Year, ParkName, Visitors) %>% 
  mutate(ParkName = str_remove(ParkName, "[A-Z]{2,}"),
         ParkName = str_remove(ParkName, "& PRES"),
         ParkName = trimws(ParkName)) %>%
  regex_left_join(park_fees, ., ignore_case = TRUE) # %>% 
  # distinct(year, ParkName, .keep_all = TRUE) %>% 
  # filter(str_detect(ParkName, "Park"), !str_detect(ParkName.x, "Great Falls")) %>% 
  # mutate(revenue = visitors * parse_number(per_person)) %>% 
  # rename(park_name = ParkName.x) %>% 
  # select(-park_name.y)
